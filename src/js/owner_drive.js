...
async function showProfessors() {
    ...
    for (let i = 0; i<n_professors; i++){
      outputString += "<p>" + await contractDrive.methods.getProfessorByIndex(i).call() + "</p>";
    }
...
form.addEventListener("submit", async function(event) {
    ...
    await fetch('src/users.json')
        ...
        if (profExists) {
            const result = await contractDrive.methods.addProfessor(profAdd).call({from: account});
            await contractDrive.methods.addProfessor(profAdd).send({from: account});
            if (result == 1) {
...
formDelete.addEventListener("submit", async function(event) {
    ...
    const result = await contractDrive.methods.deleteProfessor(profAdd).call({from: account});
    await contractDrive.methods.deleteProfessor(profAdd).send({from: account});
    if(result == 1){
